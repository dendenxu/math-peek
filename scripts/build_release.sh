...
- Markdown 终端输出把独占行的公式方括号显示成普通 [ / ] 时，仍可恢复并识别完整公式。
- 已确认的公式块若带有 Markdown 标题产生的 # 前缀，也会在渲染前清理。
- 支持 Codex TUI 把用户输入开头显示成 › [ 的公式块。
- 被吞掉的行内分隔符按数学原子、函数记号、运算结构和 TeX 命令统一识别，不依赖单条公式白名单。
- iTerm2 坐标回退仅在鼠标稳定后检查附近几行，避免全屏 bounds 搜索造成延迟。
- 前台应用通过系统切换通知缓存，不在 60 Hz 悬停循环中重复查询 LaunchServices。
- 行内公式的 \( / \) 被显示成普通括号时，含 TeX 命令、紧凑上下标或单个数学变量的内容仍可保守识别。
